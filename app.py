...
with tab_board:
    ...
    st.markdown("### <b>Strategic Play Windows</b>", unsafe_allow_html=True)
    ...
    if p_data["hand"]:
        ...
        if st.button(f"🔥 Strike Deck / Play Card: {selected_card}", use_container_width=True):
            if st.session_state.current_round == 1 and selected_card in ["Subjugation", "Absolution", "Damned Humans"]:
                st.error("🚫 SANCTION RULE: Alignment protective fields prevent forced swaps on Round 1.")
            else:
                ...
                if "White Token" in meta["effect"]:
                    ...
                    if t_data["health"] > (INITIAL_HEALTH / 2):
                        st.warning(f"🛡️ PARRIED: {target_entity} has too much health (>50% HP) to be instantly converted.")
                    else:
    ...
    if st.session_state.white_tokens >= MAX_TOKENS: st.success("🎉 HUMAN FACTION TRIUMPHS! Soul Salvaged.")
        ...
        st.info("⌛ MAX ROUNDS REACHED")
        if st.session_state.white_tokens > st.session_state.black_tokens: st.success("Humans win via majority count!")
        elif st.session_state.black_tokens > st.session_state.white_tokens: st.error("Corruption wins via majority count!")
...
with tab_rules:
    ...
    <div class="parchment-container" style="font-size:15px; line-height:1.6; text-align:justify;">
        ...
        Faction & Soul is a highly competitive, fast-paced tactical card game for 4 players divided into two shifting secret alliances: The <b>Humans (Selfless Faction)</b> and The <b>Rejected (Selfish Faction)</b>. The battlefield centers around the <i>Unwilling Child</i>, an entity being torn between purification and corruption.
        <br/><br/>
        <b>SECTION II: GAME SETUP & VITALS</b><br/>
        1. Separate the 160 cards into three discrete decks: the 70-card Selfless Deck, the 70-card Selfish Deck, and the 20-card Secret Objective Deck. Shuffle each deck thoroughly.<br/>
        2. Each player selects a double-sided Character Mat and sets their Vitality tracker to exactly <b>6 Hit Points (HP)</b>.<br/>
        3. Each player is dealt 1 Secret Objective card. This agenda remains completely hidden until game end calculation phases.<br/>
        4. Players receive 3 starting cards matching their initial alignment.
        <br/><br/>
        <b>SECTION III: THE SHIFTING ALIGNMENT SYSTEM</b><br/>
        Spiritual health is fluid. If your character's health pool is dropped to <b>0 HP</b> via cards or combat, your soul instantly shatters. You do not leave the game; instead, you immediately <b>flip your entire alignment</b> to the opposing side (Human transforms to Rejected; Rejected transforms to Human). Your health is instantly restored back to full <b>6 HP</b>, and your future card draw rules shift to your new deck alignment.
        <br/><br/>
        <b>SECTION IV: PHASES OF A TURN</b><br/>
        Every game round proceeds clockwise, player by player. A full player turn consists of:<br/>
        • <b>1. The Cast Phase:</b> Roll the 3 Custom Action Dice to generate your resource pool (Persuasion, Resistance, Manipulation).<br/>
        • <b>2. The Mat Modification Phase:</b> You may alter your raw dice outcomes by invoking any edge of your Character Mat, adjusting your resources at the cost of the corresponding directional penalty.<br/>
        • <b>3. The Action Phase:</b> Spend your resources to play cards out of your current hand, attack targets, or build toward token generation.<br/>
        • <b>4. The Cleanse Phase:</b> Pass the turn.
        <br/><br/>
        <b>SECTION V: CANONICAL BALANCING CONSTRAINTS</b><br/>
        To ensure production-grade balanced mechanics and prevent game-breaking advantages, the following structural strictures apply:<br/>
        • <b>Alignment Protection Field:</b> No instant conversion or alignment swapping actions (via cards like <i>Subjugation</i>, <i>Absolution</i>, or <i>Damned Humans</i>) may be played during **Round 1**.<br/>
        • <b>Spiritual Fortitude Gating:</b> High-tier alignment conversion cards can only target an entity whose defensive systems have been compromised. Instant alignment-altering abilities fail if the target possesses more than 50% of their base health (must be at 3 HP or lower).
        <br/><br/>
        <b>SECTION VI: THE WIN CONDITIONS</b><br/>
        The tournament concludes immediately when any of the three triggers occur:<br/>
        1. <b>The White Victory:</b> The central dashboard accumulates 10 White Tokens. The Human Faction wins.<br/>
        2. <b>The Black Victory:</b> The central dashboard accumulates 10 Black Tokens. The Rejected Faction wins.<br/>
        3. <b>The Doom Clock:</b> If the round timer expires at the end of Round 8, token majority establishes the winning faction.
    </div>
    """
    st.markdown(rulebook_html, unsafe_allow_html=True)
